/*check validate the username input*/
function check_inputUsername() {
	var text_username = $("#username").val();
	var show_err_username = $("#err_username");

	show_err_username.html("");

	if (isSpecialCharacter(text_username)) {
		show_err_username.html("not enter special character.");
		return false;
	}

	if (text_username.length < 8 || text_username.length > 50) {
		show_err_username.html("username in 8-50 character");
		return false;
	}

	show_err_username.html("OK");
	show_err_username.css({"color": "green"});
	return true;
}

/*check 1 string have special character? true : false*/
function isSpecialCharacter(str){
 return /[~`!#$%\^&*+=\-\[\]\\';,/{}|\\":<>\?]/g.test(str);
}

/*check validate the password input*/
function check_inputPassword() {
	var text_password = $("#password").val();
	var show_err_password = $("#err_password");

	if (text_password.length < 8 || text_password.length > 30) {
		show_err_password.html("Pass in 8-30 character");
		return false;
	}

	show_err_password.css({"color": "green"});
	show_err_password.html("OK");
	return true;
}

/*check validate the email input when user enter*/
function check_inputEmail() {
	var text_email = $("#email").val();
	var show_err_email = $("#err_email");

	if (!validateEmail(text_email)) {
		show_err_email.html("Email is not valid");
		return false;
	}

	show_err_email.css({"color": "green"});
	show_err_email.html("OK");
	return true;
}

/*check email validate*/
function validateEmail(email) {
  var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

  return reg.test(email);
}

/*check date format when user input*/
function check_inputBirthday() {
	var text_birthday = $("#input_calendar").val();
	var show_err_birthday = $("#err_birthday");

	if (!isValidDate(text_birthday)) {
		show_err_birthday.html("Date not format");
		return false;
	}

	show_err_birthday.css({"color": "green"});
	show_err_birthday.html("OK");
	return true;
}

/*function check validate date format.*/
function isValidDate(dateString) {
	// First check for the pattern
	var reg = /^\d{1,2}\/\d{1,2}\/\d{4}$/;

    if(!reg.test(dateString))
        return false;

    // Parse the date parts to integers
    var parts = dateString.split("/");
    var day = parseInt(parts[0], 10);
    var month = parseInt(parts[1], 10);
    var year = parseInt(parts[2], 10);

    // Check the ranges of month and year
    if(year < 1970 || year > 2099 || month < 1 || month > 12 || day < 1 || day > 31)
        return false;

   	return true;
}

/*event when user click button submit
* check all input?
* send username, password to server by ajax.
**/
function submit_check() {
	if (check_inputBirthday() && check_inputUsername() && check_inputPassword()) {
		console.log("pass");
		var username = $("#username").val();
		var password = $("#password").val();

		if (username !== "" && password !== "") {
			$.ajax({
				type: "GET",
				url: "checklogin.php",
				dataType:"html",
				data: {
					username: username,
					password: password
				},
				success: function(response) {
					$("#kq").html(response);
				}
			})
		}
	}
}

/*refresh the input and the error display element*/
function refresh() {
	$("#username").val("");
	$("#password").val("");
	$("#email").val("");
	$("#input_calendar").val("");
	$("#err_username").html("");
	$("#err_password").html("");
	$("#err_email").html("");
	$("#err_birthday").html("");
}